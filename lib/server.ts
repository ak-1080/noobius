...
async function player(wallet: string): Promise<Profile> {
  ...
  return {
    wallet: p.wallet,
    publicId: p.public_id ?? p.wallet.slice(2, 18),
    name: p.name,
    credits: p.credits,
    xp: p.xp,
    shifts: p.shifts,
  ...
  if (request.method === 'GET') {
    ...
    if (action === 'campus') {
      ...
      const people = await db()
        .prepare(
          'SELECT COALESCE(p.public_id,substr(p.wallet,3,16)) AS id,p.name,c.x,c.z,p.facility_state FROM crew_presence c JOIN players p ON p.wallet=c.wallet WHERE c.room=? AND c.updated_at>? LIMIT 30',
        )
    ...
    if (action === 'directory') {
      const rows = await db()
        .prepare(
          'SELECT COALESCE(p.public_id,substr(p.wallet,3,16)) AS id,p.name, p.facility_state FROM players p JOIN crew_presence c ON c.wallet=p.wallet WHERE c.updated_at>? ORDER BY c.updated_at DESC LIMIT 30',
        )
    ...
    if (action === 'visit') {
      ...
      if (!owner || !/^(?:[a-fA-F0-9]{16}|[a-fA-F0-9]{32})$/.test(owner))
        throw new ApiError(400, 'Choose a facility from the directory.');
      const row = await db()
        .prepare(
          'SELECT name,facility_state FROM players WHERE COALESCE(public_id,substr(wallet,3,16))=?',
        )
        .bind(owner)
        .first<any>();
    ...
    if (action === 'listings') {
      ...
      const rows = await db()
        .prepare(
          "SELECT l.id,COALESCE(p.public_id,substr(p.wallet,3,16)) AS owner,p.name,l.item,l.quantity,l.price FROM market_listings l JOIN players p ON p.wallet=l.wallet WHERE l.status='open' AND (l.wallet=? OR l.id IN (SELECT id FROM market_listings WHERE status='open' ORDER BY created_at DESC LIMIT 50)) ORDER BY l.created_at DESC",
        )
        .bind(wallet ?? '')
        .all();
