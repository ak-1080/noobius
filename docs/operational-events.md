...
| `room-authority-retry`   | A read-only authority refresh hit a transient service failure while the existing short lease was still valid. The next alarm retries; this does not extend the lease or retry an uncertain position write.                                                         |
