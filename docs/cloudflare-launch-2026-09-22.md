...
The repository also has a manual **Deploy Noobius game** GitHub Actions workflow for releases from `main`. It runs the same verified deploy command and then checks the public game, root coming-soon page and room endpoint. To activate it, add a repository Actions secret named `CLOUDFLARE_API_TOKEN` in `ak-1080/noobius` with permission to deploy the three existing Workers and apply production D1 migrations. The account ID is fixed in the workflow; the deployment script checks the game Worker and database destination. The secret is not present as of September 23; without it the workflow fails before checkout and does not deploy. The workflow is manual, so pushing to `main` alone does not publish a new game version. Keep the API token out of source and never paste it into a workflow input or log. Cloudflare's [GitHub Actions guide](https://developers.cloudflare.com/workers/ci-cd/external-cicd/github-actions/) documents this authentication pattern.
