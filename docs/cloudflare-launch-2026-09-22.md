...
2. Broader mixed-workload and measured quotas/cost. Two 50-player, ten-neighborhood full-speed movement/profile-read tests passed, including one after Workers Paid activation on September 23. A 20-room-client/three-gameplay-player overlap also passed, including a six-minute room measurement and five repair cycles per gameplay player through the renewal window. Three unfinished hosted repairs also survived a real room Worker redeploy, reconnect, and completion. See [the operations record](cloudflare-operations.md) for exact scope, one recovered unexpected disconnect in the longer room probe, and D1 snapshots. These results still do not prove sustained 50-player jobs/trades load, browser-level multiplayer, or performance above the current cap.
