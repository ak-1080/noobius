...
2. Longer mixed-workload and worker-restart acceptance, plus measured quotas/cost. A 50-player, ten-neighborhood full-speed movement/profile-read test passed with 99.83% accepted updates, p95 movement acknowledgment 101.3 ms, 16 planned renewals, no unexpected interruptions and all 50 final positions saved. See [the operations record](cloudflare-operations.md) for scope and the committed report. This does not prove sustained jobs/trades load or performance above the current 50-player cap.
3. Upgrade/confirm Workers billing before public multiplayer. At 23:15 UTC on September 22, live Worker logs explicitly reported `D1_ERROR: Your account has exceeded D1's free tier daily row read limit.` Health and marketplace reads intermittently returned 503/500 while static pages continued loading. A trivial `SELECT 1` succeeding does not disprove exhaustion because it reads no table rows. Further live load testing is held until reset or upgrade; no paid upgrade has been made. Cloudflare documents a midnight-UTC daily reset and Workers Paid starting at $5/month plus metered usage. Subscription read had returned 403; the runtime quota error is the current authoritative evidence of free-tier enforcement.
...
During the quota outage, the three-player job/visit/item-trade/relogin flow passed against isolated local Worker/D1/room processes without injected balances or accelerated clocks. The [gameplay acceptance procedure](gameplay-acceptance.md) and its script are ready for an equivalent hosted run once quota permits. This is not yet a hosted gameplay or real-wallet UI pass.
