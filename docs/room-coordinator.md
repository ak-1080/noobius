...
Originally documented September 9, 2026; hosting status updated September 22. The authoritative room Worker, browser transport, durable movement checkpoints and physical-action fencing are now deployed at `rooms.noobius.io`, with the game/economy service at `play.noobius.io`. The sections below include historical local verification; use [the Cloudflare deployment record](cloudflare-launch-2026-09-22.md) for current hosted evidence and remaining release gates.
...
The browser sends movement no more often than every 160 ms and waits at least 110 ms after the previous acknowledgment to avoid packet bunching. A server correction stops an invalid move. Background saves renew a short connection lease without snapping the character back to an older position. Jobs, shared outages, the project and neighbor roster refresh separately through a controller-checked metadata endpoint; live peer positions overlay that metadata.
...
- Movement uses its own input sequence, separate from the durable checkpoint sequence. Legal floor, obstacles, unlocked rooms, elapsed movement time, speed and input rate are checked server-side. Empty-time packet floods earn no travel distance; each hop has at most one second of travel budget. Up to 250 ms of earned, unspent server travel time can carry between accepted packets to absorb arrival jitter. Actual navigable path distance is charged against that budget. Invalid moves forfeit the remainder; work freezes and rebases clear it. There is no per-packet distance gift and no client-provided clock.
