...
| `NOOBIUS_GPU_ADMISSION_PAUSED` | `false` | Exact string `true` stops fresh GPU admission and GPU room changes, preserving free admission and safe return. Live GPU reconnects retain their existing neighborhood and slot; explicit takeover rotates the controller in place. Earned claims and escrow cancellation keep their normal rules. |
...
The GPU pause is an admission control, not entitlement revocation. Same-controller resumes preserve the current interior and movement; takeover resets movement in the same slot. Expired/missing memberships must wait. The commit checks membership identity, a live lease and the current holding predicate; it never falls back to matchmaking while paused. Configuration is a deployment/request snapshot, so it does not cancel an in-flight request that already read older settings.
...
- For a failed hosted migration or data-integrity incident, preserve error and version evidence and use a verified recovery path. These admission and activity pauses are not a universal write freeze.

Before public access, assign an operator and verify dashboards/alerts for 5xx, 429, 409, action latency, active admissions, D1 reads/writes/storage, failed logins and costs. Distinguish expected stale-controller or insufficient-resource responses from outages. Fixed-schema room recovery and holder-provider events now exist in source; see [operational events](operational-events.md) for their exact meanings and test evidence. Standalone coordinator events will run only after that service is deployed. Dedicated alert delivery, retention policy, incident ownership and cost thresholds are not yet proven. Do not log raw signatures, login challenges, session tokens, seed phrases or RPC credentials.
