...
import { applyFacility, newFacility, normalizeFacility, storedComputeNow, computeTankCapacity, ITEMS } from '../lib/facility.ts';
import { careerFor, contractTemplate, serviceChallenge, SERVICE_REPAIRS, validCareer, operatorLicense, contractQuote } from '../lib/contracts.ts';
...
function act(f, type, now, extra = {}) {
  return applyFacility(f, { type, requestId: crypto.randomUUID(), ...extra }, f.compute, now).facility;
}
function accept(f, family, now) {
  const offer = f.career.offers.find(o => contractTemplate(o.template).family === family);
...
function finish(f, id, now) {
  ...
  if (contractTemplate(r.template).family === 'service') {
    ...
    f = act(f, 'contract-service', now, { id, direction: serviceChallenge(r).answer });
    now = f.career.active[0].nextStepAt;
...
test('canceling and reaccepting keeps the same offer, with no earned progress or free rerolls', () => {
  ...
  for (let i = 0; i < 30; i++) {
    ...
    assert.throws(() => act(accepted, 'contract-accept', i, { id }), /already been taken/);
...
test('two job limit, escrow, delivery delay, and once-only payout survive retry-history eviction', () => {
  ...
  for (const [item, count] of Object.entries(run.cost)) assert.equal(f.inventory[item], before.inventory[item] - count);
  assert.throws(() => act(f, 'contract-start', 1000, { id }), /already started/);
  assert.throws(() => act(f, 'contract-cancel', 1000, { id }), /already started/);
  assert.throws(() => act(f, 'contract-claim', run.readyAt - 1, { id }), /Finish the work/);
  ...
  assert.throws(() => act(f, 'contract-claim', run.readyAt + 1, { id }), /no longer active/);
...
test('diagnosis requires observation, a correct repair and server-timed verification', () => {
  ...
  const r = f.career.active[0], expected = serviceChallenge(r).answer;
  assert.throws(() => act(f, 'contract-service', 0, { id, direction: 'Inspect' }), /finish first/);
  assert.throws(() => act(f, 'contract-service', 3000, { id, direction: expected }), /Inspect/);
  ...
  assert.throws(() => act(f, 'contract-service', 9999, { id, direction: expected }), /finish first/);
  f = act(f, 'contract-service', 10000, { id, direction: expected });
  assert.throws(() => act(f, 'contract-claim', 10000, { id }), /Finish the work/);
  assert.throws(() => act(f, 'contract-service', 10000, { id, direction: 'Test' }), /finish first/);
...
test('workload output is conserved across clock boundaries, repeated settlement, late claims and cap', () => {
  for (const start of [0, 1, 14999, 15000, 15001, 29999]) for (const full of [false, true]) {
    let f = fixture();
    f.career.offers.find(o => contractTemplate(o.template).family === 'workload').template = 'tiny-model';
    if (full) f.storedCompute = computeTankCapacity(f);
    let id; [f, id] = accept(f, 'workload', start);
    f = act(f, 'contract-start', start, { id, rack: 'rack-a' });
    const r = f.career.active[0], before = f.storedCompute;
    assert.equal(r.reward, 32 + 4 * 6);
    assert.throws(() => act(f, 'compute-upgrade', start, {}), /running client workloads/);
    ...
    for (let at = start + 7000; at < end; at += 7000) f = act(f, 'contract-track', at, { id });
    assert.equal(storedComputeNow(f, end), Math.min(computeTankCapacity(f), before + 4 * 6));
    f = act(f, 'contract-claim', end + 30000, { id });
    assert.equal(f.compute, r.reward);
    assert.equal(f.storedCompute, Math.min(computeTankCapacity(f), before + 4 * 6 + 2 * 12));
    assert.ok(validCareer(f.career));
  }
});
...
test('module choices alter committed inputs/time; running jobs keep their configuration', () => {
  ...
  const offer = f.career.offers.find(o => contractTemplate(o.template).family === 'workload');
  offer.template = template.id;
  f = act(f, 'contract-accept', 0, { id: offer.id });
  assert.throws(() => act(f, 'contract-start', 0, { id: offer.id, rack: 'rack-a', direction: 'stable' }), /Equip/);
  f = act(f, 'contract-start', 0, { id: offer.id, rack: 'rack-a', direction: 'fast' });
...
test('off-grid harvest from a full tank cannot change the production phase or overpay a workload', () => {
  for (const harvestAt of [1, 1000, 14000]) {
    ...
    f.career.offers.find(o => contractTemplate(o.template).family === 'workload').template = 'tiny-model';
    let id; [f, id] = accept(f, 'workload', 0);
...
test('a fully upgraded player still has renewable offers and cannot purchase qualification', () => {
  let f = fixture(), now = 0;
  f.builds = Object.fromEntries('abcdefg'.split('').map(id => ['rack-' + id, 3]));
  f.computeBoost = 5;
  f.compute = 1000000;
  f.unlocked = ['commons', 'salvage', 'workshop', 'thermal', 'compute', 'network', 'core'];
  f.stats.gathered = 50; f.stats.crafted = 20;
  const seen = new Set();
  assert.equal(operatorLicense(f.career), false);
  assert.throws(() => act(f, 'module-build', now, { id: 'fast' }), /Complete 2/);
  for (let i = 0; i < 36; i++) {
    let id; [f, id] = accept(f, ['service', 'supply', 'workload'][i % 3], now);
  ...
  assert.equal(operatorLicense(f.career), false, 'Commission a shared cluster to finish the license');
...
test('legacy saves retain property and resources while gaining a fresh career', () => {
  ...
  f.inventory = { kit: 4 }; f.owned.push('mint'); f.compute = 41000;
  f.builds = { 'rack-a': 3 }; f.craft = { recipe: 'kit', readyAt: 10000 };
  const before = structuredClone(f), upgraded = normalizeFacility(f, 1000);
  for (const key of ['inventory', 'owned', 'compute', 'builds', 'craft']) assert.deepEqual(upgraded[key], before[key]);
  ...
  for (const item of ['kit', 'board', 'battery', 'core']) assert.throws(() => act(upgraded, 'buy', 1000, { item, quantity: 1 }), /Craft equipment/);
});

test('all guidance discards commands; following a direction cannot claim or buy anything', () => {
  for (const type of ['build', 'craft', 'gather', 'contract-claim', 'compute-harvest', 'unlock']) {
    const next = { title: 'Useful work', detail: '', cta: 'Do it', target: 'rack-a', action: { type, id: 'rack-a' }, repair: 'boot' };
    const before = structuredClone(next), guide = guidanceFor(next);
