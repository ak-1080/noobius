...
- **Staging:** `https://noobius-game-staging.rinkydooonso.workers.dev` has an isolated Cloudflare D1 database and Solana **devnet** checkout for valueless test tokens. The September 25 active-play pass is deployed here, after a private staging D1 export under ignored `.wrangler/staging-backups/`. The guarded deployment and a three-player normal-action smoke passed, including finite batch issuance, trade, reconnect and save persistence. On September 24, two human players completed a browser-wallet sale: the buyer paid one devnet test token, received 100 Compute, and the seller's offer was marked sold. This proves one real two-player checkout path worked before this update; the new staging build still needs a human wallet checkout. MetaMask's devnet payment prompt was misleadingly labeled Mainnet, so staging blocks MetaMask checkout; Phantom in Devnet mode completed the earlier sale.
...
On the production build, Margo introduces one free machine and machines still produce Compute every 15 seconds. **The staging active-play update changes this:** after migration, machines process finite, supplied batches and stop when those batches finish. Players recover parts, choose a batch or client order, collect the result and decide what to build or trade next. Machine levels add client capacity; five existing speed upgrades shorten newly started machine batches. Old earned storage and already accepted jobs remain intact. Production has not changed.
